...
import { useAdminGallery, useUpdateAdminItem, useAdminCategories } from "@/lib/queries";

export const Route = createFileRoute("/admin/content/gallery/$id")({
...
function EditGalleryItem() {
  ...
  const { data: gallery, isLoading: isFetching } = useAdminGallery();
  const { data: categories } = useAdminCategories();
  const galleryCategories = categories?.filter((c: any) => c.type === "gallery");

  const updateGal = useUpdateAdminItem("gallery", "gallery");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    updateGal.mutate({ id, data: formData }, {
      onSuccess: () => {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-display text-3xl font-bold">Edit Galeri</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 md:grid-cols-2">
        <div className="space-y-6">
          ...
          <Button type="submit" className="w-full rounded-full nailong-glow" size="lg" disabled={updateGal.isPending}>
            {updateGal.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
            Simpan Perubahan
          </Button>
        ...
        <div>
          <Card className="p-6 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-hidden rounded-xl bg-muted border border-border flex items-center justify-center">
              <img src={formData.img || "https://placehold.co/800"} alt="Preview" className="h-full w-full object-contain" />
            </div>
          </Card>
        </div>
      </form>
